...
export function FileUpload({
	...
	// NOTE: Using <button type="button"> for WCAG compliance — native button
	// provides keyboard activation (Enter/Space) and correct ARIA semantics for free.
	...
	return (
		<div className="space-y-2">
			{/* NOTE: biome-ignore lint/a11y/useSemanticElements: drop zone needs flex block layout
			    with drag-and-drop handlers — <button> cannot receive drag events on all browsers,
			    but the native button element is used for correct focus/keyboard semantics */}
			<button
				type="button"
				disabled={disabled}
				onDrop={handleDrop}
				...
				<div className="text-sm text-muted-foreground">
					{isDragging ? (
						...
						<>
							...
							{accept && (
								<span className="block text-xs mt-1">Accepted: {accept}</span>
							)}
							{maxSize && (
