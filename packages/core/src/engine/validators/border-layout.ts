...
function validateBorderConstraints(
	...
	for (let i = 0; i < data.regions.length; i++) {
		...
		if (
			...
			typeof (region as Record<string, unknown>).position !== "string"
		) {
			ctx.addIssue({
				code: z.ZodIssueCode.custom,
	...
	if (centerCount === 0) {
		ctx.addIssue({
			code: z.ZodIssueCode.custom,
			message:
				'Border layout requires exactly one region with position "center"',
			path: ["regions"],
		});
...
export function validateBorderLayout(schema: LayoutSchema): ValidationResult {
	...
	if (centerCount === 0) {
		...
		errors.push(
			`Border layout requires exactly one "center" region, found ${centerCount}`,
		);
	}
