...
export const validationRuleSchema = z.object({
  type: z.enum(['required', 'min', 'max', 'minLength', 'maxLength', 'pattern', 'email', 'custom']),
...
export const datePickerConfigSchema = z
  ...
  .refine(
    data => {
      if (data.minDate && Number.isNaN(Date.parse(data.minDate))) return false
      if (data.maxDate && Number.isNaN(Date.parse(data.maxDate))) return false
      if (data.minDate && data.maxDate && new Date(data.minDate) > new Date(data.maxDate))
        return false
      return true
    },
    {
...
export const virtualScrollConfigSchema = z
  .object({
