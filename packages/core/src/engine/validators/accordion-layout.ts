...
export const accordionLayoutSchema = z
	.object({
		...
		regions: z
			.array(z.unknown())
			.min(1, "Accordion layout requires at least one region"),
...
function validateDefaultOpenRefs(
	...
	if (
		!accordionConfig ||
		typeof accordionConfig !== "object" ||
		accordionConfig === null
	)
		return;
...
export function validateAccordionLayout(
	schema: LayoutSchema,
): ValidationResult {
	...
	if (!Array.isArray(schema.regions)) {
		return {
			success: false,
			errors: ["Accordion layout requires a regions array"],
		};
	}
	...
	if (schema.regions.length === 0) {
		return {
			success: false,
			errors: ["Accordion layout requires at least one region"],
		};
	}
	...
	for (const entry of ids) {
		if (seen.has(entry.id)) {
			errors.push(
				`Duplicate region id "${entry.id}" at regions[${entry.index}]`,
			);
		}
		seen.add(entry.id);
	}
...
function validateMultipleDefaultOpenRuntime(
	...
	for (const openId of defaultOpen) {
		if (typeof openId !== "string") {
			errors.push(
				`accordionConfig.defaultOpen contains a non-string value (got ${typeof openId})`,
			);
			continue;
		}
		if (!idSet.has(openId)) {
			errors.push(
				`accordionConfig.defaultOpen "${openId}" does not match any region id`,
			);
		}
	}
}
