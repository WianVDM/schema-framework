...
const cardGridConfigSchema = z
	.object({
		columns: z
			.union([z.number().int().positive(), responsiveColumnsSchema])
			.optional(),
...
export const cardLayoutSchema = z
	.object({
		...
		regions: z
			.array(z.unknown())
			.min(1, "Card layout requires at least one region"),
...
export function validateCardLayout(schema: LayoutSchema): ValidationResult {
	...
	if (schema.regions.length === 0) {
		return {
			success: false,
			errors: ["Card layout requires at least one region"],
		};
	}
...
function validateColumns(
	columns: number | CardGridResponsiveColumns,
	errors: string[],
): void {
	...
	for (const key of keys) {
		...
		if (value === undefined) {
			...
			errors.push(
				`Card grid columns for breakpoint "${key}" must be a positive integer >= 1`,
			);
		}
	}
}
