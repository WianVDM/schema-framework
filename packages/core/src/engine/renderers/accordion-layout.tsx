...
export function AccordionLayoutRenderer({
	...
	const effectiveDefaultValue =
		accordionType === "single" ? (defaultOpen[0] ?? "") : defaultOpen;
...
): config is SingleAccordionConfig {
...
function FallbackAccordionLayout({
	...
	useEffect(() => {
		...
		const observer = new ResizeObserver((entries) => {
			...
			setPanelHeights((prev) => {
				...
				return Object.keys(merged).some((k) => merged[k] !== prev[k])
					? merged
					: prev;
			});
		});
	...
	const animationStyle =
		animation === "none" ? undefined : { transitionDuration: "200ms" };
