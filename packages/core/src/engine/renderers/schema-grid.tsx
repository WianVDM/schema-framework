...
import type { GridColumnSchema, SchemaGridProps, VirtualScrollConfig } from '../types'
...
export function SchemaGrid({
  ...
  const { Table, TableHeader, TableBody, TableRow, TableCell, Badge } = usePrimitives()
  ...
  useEffect(() => {
    ...
    setColumnOrder(prev => {
      ...
      return added.length > 0 || existing.length !== prev.length ? [...existing, ...added] : prev
    })
  }, [schema.columns])
  ...
  const paginationConfig =
    typeof schema.pagination === 'object' ? schema.pagination : { pageSize: 10 }
  ...
  const getRowId = useCallback(
    (row: RowData): string => resolveRowId(row, schema.dataKey, rowIdMapRef, rowIdCounterRef),
    [schema.dataKey],
  )
  ...
  const renderRow = (row: Row<RowData>, rowIndex: number) => (
    ...
    >
      {row.getVisibleCells().map(cell => (
        <TableCell key={cell.id} className={`text-sm ${cellBorderClasses}`} role="cell">
...
function resolveRowId(
  ...
  if (
    keyValue != null &&
    (typeof keyValue === 'string' || typeof keyValue === 'number' || typeof keyValue === 'boolean')
  ) {
...
function buildRowClasses(hoverable: boolean | undefined, striped: boolean | undefined): string {
  return [
...
function buildColumns(
  ...
  return columns.map(col => ({
    ...
    cell: (info: CellContext<RowData, unknown>) => renderCellValue(col, info.getValue(), Badge),
