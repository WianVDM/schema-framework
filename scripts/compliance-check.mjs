    ...
    try {
      ...
      if (context.files) {
        for (const [fileName, meta] of Object.entries(context.files)) {
          if (meta.desc && isGenericDescription(meta.desc, meta.export)) {
            const relPath = relative(ROOT, contextPath).replace(/\\/g, '/')
            suggest(`Description needed: ${relPath} → ${fileName} (has generic desc "${meta.desc}")`)
          }
        }
      }
...
function isGenericDescription(desc, exportName) {
  if (!desc) return true
  // Description is just the export name itself
  if (desc === exportName) return true
  // Auto-generated multi-export description
  if (desc.match(/^Exports \d+ symbols?$/)) return true
  // Placeholder descriptions
  if (desc.match(/^\[.+\]$/)) return true
  // Very short descriptions that aren't informative
  if (desc.length < 10 && !desc.includes(' ')) return true
  return false
}
