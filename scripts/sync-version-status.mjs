...
function readCoreVersion() {
	try {
		...
		console.error(
			`ERROR: readCoreVersion failed for ${CORE_PKG_PATH}: ${err.message}`,
		);
		return null;
	}
}
...
function nextPatch(version) {
	if (!/^\d+\.\d+\.\d+$/.test(version)) {
		throw new Error(
			`Invalid semver core version: "${version}" (expected format X.Y.Z with no prerelease/build metadata)`,
		);
	}
...
function syncVersionStatus() {
	...
	if (!coreVersion) {
		console.error(
			"ERROR: Could not read version from packages/core/package.json",
		);
		process.exit(1);
	}
	...
	if (!currentVersionMatch) {
		console.error(
			'ERROR: Could not parse "Current Version" from VERSION_STATUS.md',
		);
		process.exit(1);
	}
	...
	if (!targetVersionMatch) {
		console.error(
			'ERROR: Could not parse "Target Version" from VERSION_STATUS.md',
		);
		process.exit(1);
	}
	if (targetVersionMatch[1] !== newTarget) {
		content = content.replace(
	...
	if (changed) {
		...
		console.log(
			`Synced VERSION_STATUS.md: Current=${coreVersion}, Target=${newTarget}`,
		);
	} else {
